...
Let rho(x)={x} and rho_a(x)=rho(1/(a x)) on (0,infinity). For 0<Re(s)<1, the Mellin identity is

M rho_a(s) = -a^{-s} zeta(s)/s.

On Re(s)=1/2, Mellin–Plancherel gives, for finite coefficient vectors c,
...
## Cross-route target
The strongest next target is therefore:
