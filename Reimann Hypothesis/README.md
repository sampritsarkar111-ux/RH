...
This folder records a rigorous research attempt based on a Pólya-frequency / total-positivity reformulation of RH, plus an adversarial audit of a recent claimed closure.
...
- `2026-09-16_adversarial_audit.md` — adversarial audit identifying a circular step in a recent claimed RH proof

The July 2026 preprint by Wojciech Michalowski proves positivity of consecutive Toeplitz minors in the tail `k >= 10^18 r^3`, while explicitly leaving the complementary region as the RH-relevant obstruction. The audit file records why a separate claimed all-order spectral-gap closure cannot currently be accepted: it uses the global representation of every generating-function zero as `-t_k^2`, which presupposes the critical-line location of all zeros.
